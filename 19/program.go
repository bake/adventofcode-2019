...
func (p *program) next(ops ...operation) bool {
	for {
		op, err := p.step()
		if err != nil {
			return false
...
func (p *program) put(n int64) {
	p.input = append(p.input, n)
}

func (p *program) take(n int) (out []int64, err error) {
	for i := 0; i < n; {
		op, err := p.step()
		if err != nil {
			return nil, err
		...
		case output:
			i++
		case halt:
			return nil, fmt.Errorf("program halted")
		}
	}
	out, p.output = p.output, nil
	return out, nil
}
...
func (p *program) step() (operation, error) {
	...
	case halt:
		break
	default:
		return 0, fmt.Errorf("unknown opcode at %d: %s", p.ip, op)
	}
	return op, nil
}
